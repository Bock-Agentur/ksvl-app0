...
const generateSlots = (totalSlots: number, bookedSlots: number, craneOperators: User[]): Slot[] => {
  ...
  while (slots.length < totalSlots && attempts < maxAttempts) {
    ...
    slots.push({
      ...
      date: dateString,
      time,
      duration: [30, 45, 60][Math.floor(Math.random() * 3)] as 30 | 45 | 60,
      craneOperator: {
        id: craneOperator.id,
      ...
      for (const structure of shuffledStructures) {
        ...
        if (structure.type === 'superblock') {
          ...
          for (let blockIndex = 0; blockIndex < actualBlockSize; blockIndex++) {
            ...
            const blockDuration = Math.random() > 0.7 ? 45 : 60; // 30% 45min, 70% 60min für Blocks
            daySlots.push({
              id: `slot-${++slotIdCounter}`,
              date: dateString,
              time,
              duration: blockDuration as 45 | 60,
          ...
          for (let blockIndex = 0; blockIndex < actualBlockSize; blockIndex++) {
            ...
            const blockDuration = Math.random() > 0.5 ? 45 : 60; // 50/50 für normale Blocks  
            daySlots.push({
              id: `slot-${++slotIdCounter}`,
              date: dateString,
              time,
          ...
          const duration = durations[Math.floor(Math.random() * durations.length)]; // Zufällige Dauer
          const time = timeSlots[currentTimeIndex++];
          ...
          daySlots.push({
            ...
            date: dateString,
            time,
            duration: duration, // Zufällige Dauer
            craneOperator: operator,
            isBooked,
            bookedBy: bookedMember?.name,
            member: bookedMember,
