...
export function AppShell({
  ...
  return <>
      ...
      {/* Header */}
      <header className={cn("sticky top-0 z-40 bg-card/90 backdrop-blur-sm border-b border-border px-4 py-3 shadow-card-maritime transition-transform duration-300 ease-in-out", isHeaderVisible ? "translate-y-0" : "-translate-y-full")}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Login/Logout Button */}
            {currentUser ? <Button variant="ghost" size="sm" onClick={handleLogout} className="gap-2">
                <LogOut className="w-4 h-4" />
                <span className="hidden sm:inline">Abmelden</span>
              </Button> : <Button variant="ghost" size="sm" onClick={() => navigate('/auth')} className="gap-2">
            ...
            <Sheet open={isMenuOpen} onOpenChange={setIsMenuOpen}>
              <SheetTrigger asChild>
                <button className="relative p-2 hover:bg-accent rounded-md transition-colors">
                  ...
                  {availableHeaderItems.some(item => item.badge) && <Badge variant="destructive" className="absolute -top-1 -right-1 h-4 w-4 text-xs p-0 flex items-center justify-center">
                ...
                <div className="mt-6 space-y-6 pb-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-col gap-2">
                      {currentUser?.roles && currentUser.roles.length > 0 ? <div className="flex flex-wrap gap-1">
                          {currentUser.roles.map(role => <Badge key={role} className={cn("text-xs", role === currentRole ? roleColors[role] : "bg-muted text-muted-foreground")}>
                              {roleLabels[role]}
                              {role === currentRole && " (aktiv)"}
                            </Badge>)}
                        </div> : <Badge variant="outline" className="text-xs w-fit">
                          Keine Rollen
                        </Badge>}
                      {currentUser && <>
                  ...
                  {availableHeaderItems.length > 0 && <>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-1">
                          ...
                          return <Button key={item.id} variant={isActive ? "secondary" : "ghost"} className="w-full justify-start relative" onClick={() => {
                            onTabChange(item.id);
                            setIsMenuOpen(false);
                          }}>
                                <Icon className="w-4 h-4 mr-2" />
                                <span>{item.label}</span>
                                {item.badge && <Badge variant="destructive" className="ml-auto h-5 w-5 text-xs p-0 flex items-center justify-center">
                                    {item.badge}
                                  </Badge>}
                              </Button>;
                        })}
      ...
      {/* Main Content */}
      <main className="flex-1 overflow-auto pb-20 pt-5 mx-0 px-0 py-0">
        {children}
      </main>
