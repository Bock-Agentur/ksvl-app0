...
export function Dashboard({ onNavigate, displayName }: DashboardProps) {
  ...
  const quickActions = useMemo(() => {
    const baseActions = [
      ...
      },
      {
        ...
        icon: Users,
        action: () => onNavigate?.("calendar"),
        roles: ["mitglied", "gastmitglied"],
      },
      {
        id: "manage-slots",
        label: "Slots verwalten",
        icon: Clock,
        action: () => onNavigate?.("slots"),
        roles: ["kranfuehrer", "admin", "vorstand"],
      },
      {
        id: "manage-users",
  ...
  const slotStats = useMemo(() => {
    ...
    return {
      ...
      utilization:
        slots && slots.length > 0 ? Math.round((slots.filter((s) => s.isBooked).length / slots.length) * 100) : 0,
    };
  }, [slots]);
  ...
  const nextBooking = useMemo(() => {
    ...
    const userBookings =
      slots
        ?.filter((slot) => slot.isBooked && slot.memberId === currentUser?.id && new Date(slot.date) >= today)
  ...
  const recentActivity = useMemo(() => {
    return (
      slots?.slice(0, 5).map((slot, index) => ({
        ...
        message: slot.isBooked ? `Neue Buchung für ${slot.date}` : `Slot verfügbar: ${slot.date}`,
        time: slot.time,
